 ...
 * @LastEditors: 幺五六
 * @LastEditTime: 2020-08-24 09:49:57
 */
...
export default class FlyLine2 {
   ...
   */
  addLine(start, end, heightLimit, flyTime, lineStyle) {
   ...
   */
  addLineBatch(positions, heightLimit, flyTime, lineStyle) {
    if (!positions || !positions.length) return;
    positions.forEach(position => {
      this.addLine(position.start, position.end, heightLimit, flyTime, lineStyle);
    });
  }
}